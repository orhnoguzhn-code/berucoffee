...
const TAB_META: Record<TabName, { icon: string; activeIcon: string; labelKey: string; accessibilityKey: string }> = {
  Home: { icon: 'home', activeIcon: 'home', labelKey: 'tabs.home', accessibilityKey: 'tabs.home' },
  Order: { icon: 'menu', activeIcon: 'menu', labelKey: 'tabs.menu', accessibilityKey: 'tabs.menu' },
  Pay: { icon: 'qr', activeIcon: 'qr', labelKey: 'tabs.pay', accessibilityKey: 'tabs.pay' },
  Rewards: { icon: 'star', activeIcon: 'starFill', labelKey: 'tabs.rewards', accessibilityKey: 'tabs.rewards' },
  Account: { icon: 'account', activeIcon: 'account', labelKey: 'tabs.account', accessibilityKey: 'tabs.account' },
};
...
function TabBar({ state, navigation }: any) {
  ...
  const animatePay = (toValue: number) => {
    Animated.spring(payScale, {
      toValue,
      useNativeDriver: true,
      speed: 45,
      bounciness: toValue === 1 ? 8 : 3,
    }).start();
  };
          ...
          if (isPay) {
            return (
              <TouchableOpacity
                key={route.key}
                onPress={onPress}
                onPressIn={() => animatePay(0.88)}
                onPressOut={() => animatePay(1)}
                activeOpacity={0.9}
                style={styles.payBtnWrap}
                accessibilityRole="button"
                accessibilityLabel={t(meta.accessibilityKey)}
...
const styles = StyleSheet.create({
  tabBarWrap: { backgroundColor: 'transparent', position: 'relative' },
  tabBar: {
    flexDirection: 'row',
  ...
  tab: { flex: 1, alignItems: 'center', paddingBottom: 12, minHeight: 58 },
  iconWrap: { width: 48, height: 32, borderRadius: 999, alignItems: 'center', justifyContent: 'center' },
  iconWrapActive: { backgroundColor: colors.primary, transform: [{ scale: 1.05 }], ...shadow.sm },
  ...
  activeDot: { position: 'absolute', bottom: 6, width: 4, height: 4, borderRadius: 2, backgroundColor: 'transparent' },
  activeDotOn: { backgroundColor: colors.primary },
  payBtnWrap: { flex: 1, alignItems: 'center', marginTop: -46, zIndex: 10, minHeight: 84 },
  payBtn: {
    width: 70,
    height: 70,
    borderRadius: 35,
